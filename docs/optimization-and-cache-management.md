...
| 최적화 | 효과 | 누적 절감 |
|--------|------|----------|
| TTS 캐싱 | 반복 응답 0.9초 (API 호출 안 함) | 0.9초 |
| TTS_DELAY | 재생 시작 300ms 단축 | 0.3초 |
| Audio Context | 메모리 누수 방지, 배터리 절감 | - |

### 전체 응답 시간

```
최적화 전:  5-7초 (Chat 1초 + TTS 2-3초 + 재생 2-3초)
   ↓
최적화 후:  4-6초
  - 캐싱 사용 시: 2-3초 (반복 응답)
