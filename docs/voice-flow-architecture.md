...
MP3는 헤더와 프레임으로 구성되므로 단순 병합이 불가능:
...
**해결 방법: 전체 텍스트를 한번에 TTS 처리**
```typescript
// 분절 + 배치 처리 제거
const audioBlob = await handleTTSAPI(aiResponse)  // 전체 텍스트 한 번에
```

✅ 장점:
- MP3 파일 하나 생성 (병합 문제 없음)
- 간단함
- 음성 깨끗함

---

### 문제 2: AudioContext로 인한 음성 왜곡 (해결됨 ✅)

**원인:**
```
AudioPlayer.tsx에서 TTS 재생할 때 AudioContext 실행
  ↓
매번 새로운 AudioContext 생성
  ↓
createMediaElementAudioSource 중복 호출
  ↓
음성 왜곡 + 지지직거림
```

**AudioContext의 역할:**
- STT (마이크): 음성 크기 분석 → 원 크기 변화 (시각 효과)
- TTS (읽어주기): 불필요 (오히려 해로움)

**해결 방법: TTS에서 AudioContext 제거**
```typescript
// AudioPlayer.tsx에서 updateVolume 함수 제거
// scale state 제거
// 순수 오디오 재생만 유지
```
...
- TTS 음성이 깨끗해짐
- 원은 고정 (시각 효과 없음)
- PulseIndicator가 STT 시각 효과 담당

---

### 기능 3: 음성 재생 속도 조절 (구현됨 ✅)

**방법: HTML Audio playbackRate 사용**
```typescript
audioRef.current.playbackRate = 1.2  // 20% 빠름
```
